...
namespace RDST
{
   ...
   Sphere::intersect(const Ray& ray) const
   {
      ...
      if (s < 0.f && ll > rr) return IntersectionPtr(new Intersection()); //sphere is behind us and we're not inside
      float mm = ll-(s*s); //dist from sphere center projected onto ray to sphere center
      if (mm > rr) return IntersectionPtr(new Intersection()); //ray misses (sphere center projected onto ray - sphere center > radius)
      float q = sqrtf(rr-mm);
      float t = 0.f;
      ...
      return IntersectionPtr(new Intersection(true, t, ray.o + (ray.d*t), glm::normalize(n), Surface(getColor(), getFinish())));
   }
   ...
   Plane::intersect(const Ray& ray) const
   {
      ...
      if (denom == 0.f) return IntersectionPtr(new Intersection()); //ray is parallel
      float t = -(glm::dot(n, xr.o) - getDistance()) / denom;
      if (t < 0.f) return IntersectionPtr(new Intersection()); //ray intersected behind us
      if (denom > 0.f) n = -n; //flip normal if we're under the plane
      return IntersectionPtr(new Intersection(true, t, ray.o + (ray.d*t), n, Surface(getColor(), getFinish())));
   }
   ...
   Triangle::intersect(const Ray& ray) const
   {
      ...
      if (a == 0.f) return IntersectionPtr(new Intersection());
      //compute denominator
      float f = 1.f/a;
      //compute barycentric coordinates
      glm::vec3 s = xr.o - getVertex0();
      float u = f*glm::dot(s, p);
      if (u < 0.f || u > 1.f) return IntersectionPtr(new Intersection());
      glm::vec3 q = glm::cross(s, e1);
      float v = f*glm::dot(xr.d, q);
      if (v < 0.f || u+v > 1.f) return IntersectionPtr(new Intersection());
      //compute line parameter
      float t = f*glm::dot(e2, q);
      return IntersectionPtr(new Intersection(true, t, ray.o+(ray.d*t), getNormal(), Surface(getColor(), getFinish())));
   }
}